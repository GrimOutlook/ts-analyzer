...
impl TrackedPayload {
    ...
    pub fn new(pid: u16) -> Self {
        TrackedPayload { pid, payloads: Vec::new() }
    }
    ...
    ///
    /// If there are no payloads currently stored, and we are trying to add a
    /// payload that does not have the `PUSI` set, we do not add it as we
    /// will not be able to extract a full payload without the first payload
    /// that has the `PUSI` set.
    ...
    /// Adds raw payload bytes from a TSPayload object and returns a completed
    /// payload if one exists
    pub fn add_and_get_complete(
        &mut self,
        payload: &TSPayload,
    ) -> Option<Box<[u8]>> {
        self.add(payload);
    ...
    /// Check to see if there is a completed payload in the payloads vector and
    /// return the completed payload data if there is.
    pub fn get_completed(&mut self) -> Option<Box<[u8]>> {
        // Find the first payload with a start index.
        let start_partial_payload =
            self.payloads.iter().position(|payload| payload.is_start())?;
        let end_partial_payload =
            self.payloads.iter().rposition(|payload| payload.is_start())?;

        // If the indices are the same then we cannot determine if all the
        // payload data has been found.
        ...
        // Retrieve the data after the start index from the first partial
        // payload.
        let start_data =
            self.payloads[start_partial_payload].get_start_data().unwrap();
        #[cfg(feature = "log")]
        ...
        // Remove all of the payloads that have just been read, except the last
        // one. The last one will have data that pertains to the next
        // payload.
        for _ in start_partial_payload..end_partial_payload {
            self.payloads.remove(start_partial_payload);
        }

        // TODO: Investigate changing this `.into_vec()` call to something else.
        // This is the only way I could get it to work and it's likely
        // that this has performance impacts.
        let payload_data: Box<[u8]> =
            data_vec.iter().flat_map(|s| s.clone().into_vec()).collect();
